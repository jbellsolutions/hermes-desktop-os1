#!/usr/bin/env python3
"""Install local SSH pubkey on a DO droplet via authenticated noVNC console."""
...
async (lines) => {
  ...
  const waitConnected = async () => {
    for (let i = 0; i < 60; i++) {
      ...
      if (state === "connected" || state === "normal") return;
      await sleep(500);
    }
    throw new Error("VNC never connected");
  };
  const key = (keysym, down = true) => rfb.sendKey(keysym, null, down);
  const tap = (keysym) => {
    key(keysym, true);
    key(keysym, false);
  };
  ...
  for (const line of lines) {
    await typeText(line);
    await enter();
    await sleep(line === lines[0] ? 2500 : 2000);
  }
  return "typed";
}
"""
...
def run_console(droplet_id: int, root_pass: str, install_cmd: str) -> None:
    cookies = chrome_cookies()
    if not cookies:
        raise RuntimeError("No digitalocean.com cookies in Chrome")

    lines = ["root", root_pass, root_pass, root_pass, root_pass, install_cmd]
    ...
    with sync_playwright() as p:
        ...
        page.screenshot(path="/tmp/do-vnc-install-key.png", full_page=True)
        browser.close()
...
def main() -> int:
    ...
    run_console(args.droplet_id, args.root_pass, install)
    time.sleep(2)
    if verify_ssh(args.ip):
        print(f"SSH OK for root@{args.ip}")
        return 0
    print(f"SSH still failing for root@{args.ip}", file=sys.stderr)
    return 1
